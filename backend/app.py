...
PUBLIC_OFFICE_INFO = {
    ...
    "subtitle": "Public Mission Control for an AI-native company",
    "gateway_label": "Reception AI / Gateway",
    "gateway_host": "bot.deepnoa.com",
    "human_host": "deepnoa.com",
}
...
PUBLIC_SYSTEMS = [
    ...
    {"name": "Task-Noa", "status": "connected"},
    {"name": "Google Workspace", "status": "connected"},
    {"name": "NAS", "status": "connected"},
]
...
PUBLIC_AGENT_CATALOG = [
    {
        ...
        "name": "Reception AI",
        "role": "Gateway",
        "description": "Routes inbound requests and presents public-safe office state.",
    },
    {
        "key": "dev",
        "name": "Dev Agent",
        "role": "Engineering",
        "description": "Reviews code, prepares changes, and tracks implementation work.",
    },
    {
        "key": "research",
        "name": "Research Agent",
        "role": "Research",
        "description": "Scans the web and condenses findings into usable briefings.",
    },
    {
        "key": "ops",
        "name": "Ops Agent",
        "role": "Operations",
        "description": "Monitors systems, sync jobs, and infrastructure health.",
    },
    {
        "key": "support",
        "name": "Support Agent",
        "role": "Support",
        "description": "Keeps workflows responsive and maintains user-facing responses.",
    },
]
...
def get_office_name_from_identity():
    """Read a plain office display name from OpenClaw workspace IDENTITY.md."""
    ...
    try:
        ...
        if m:
            ...
            if not name or "pick something you like" in name.lower():
                return None
            return name or None
    except Exception:
        pass
...
def gateway_page():
    """Serve the bot.deepnoa.com style gateway page."""
    with open(FRONTEND_GATEWAY_FILE, "r", encoding="utf-8") as f:
        html = f.read()
    resp = make_response(html.replace("{{VERSION_TIMESTAMP}}", VERSION_TIMESTAMP))
    resp.headers["Content-Type"] = "text/html; charset=utf-8"
    return resp
...
def _default_role_state(role_key: str) -> dict:
    ...
    return {
        ...
        "updated_at": datetime.now().isoformat(),
        "public_status_label": role["public_status_labels"]["idle"],
...
def _default_manager_state() -> dict:
    roles = {role_key: _default_role_state(role_key) for role_key in ROLE_DEFINITIONS}
    return {
        "version": 1,
        "updated_at": datetime.now().isoformat(),
        "gateway": {
        ...
        "roles": roles,
        "fallback_worker": {
            "key": UNIVERSAL_FALLBACK["key"],
            "name": UNIVERSAL_FALLBACK["name"],
            "role": UNIVERSAL_FALLBACK["role"],
            "profile": UNIVERSAL_FALLBACK["profile"],
            "allowed_tools": list(UNIVERSAL_FALLBACK["allowed_tools"]),
            "status": "available",
        },
        "activity": [],
        "intake": [],
    ...
    if os.path.exists(MANAGER_STATE_FILE):
        try:
            ...
            if isinstance(data, dict):
                merged = _default_manager_state()
                merged.update({k: v for k, v in data.items() if k != "roles"})
                roles = merged["roles"]
                for role_key in roles:
                    if isinstance(data.get("roles", {}).get(role_key), dict):
                        roles[role_key].update(data["roles"][role_key])
                return merged
        except Exception:
            pass
    return _default_manager_state()
...
def _classify_agent_role(name: str, detail: str, is_main: bool) -> str:
    ...
    if is_main or "reception" in text or "gateway" in text or "受付" in text:
        return "reception"
    if "dev" in text or "review" in text or "code" in text or "開発" in text:
        return "dev"
    if "research" in text or "調査" in text or "search" in text:
        return "research"
    if "ops" in text or "monitor" in text or "監視" in text or "sync" in text:
        return "ops"
    if "support" in text or "help" in text or "案内" in text:
        return "support"
    return "support"
...
def _sanitize_public_detail(detail: str) -> str:
    ...
    text = re.sub(r"\b[\w.-]+\/[\w.-]+\b", "[private-repo]", text)
    text = re.sub(r"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b", "[private-email]", text)
    text = re.sub(r"\b(?:sk|ghp|ghu|github_pat|xoxb|AIza)[A-Za-z0-9._-]+\b", "[secret]", text)
    return text[:140]
...
def _public_status_for_role(role_key: str, state: str) -> str:
    labels = ROLE_DEFINITIONS.get(role_key, {}).get("public_status_labels", {})
    return labels.get(_state_bucket(state), _agent_status_badge(state))


def route_manager_event(payload: dict) -> tuple[str, str]:
    explicit_role = str(payload.get("role") or "").strip().lower()
    if explicit_role in ROLE_DEFINITIONS:
...
def apply_manager_event(payload: dict) -> dict:
    ...
    if routed_role in ROLE_DEFINITIONS:
        role_state = manager["roles"][routed_role]
        role_state["state"] = state
        role_state["detail"] = safe_summary
        role_state["updated_at"] = timestamp
        role_state["public_status_label"] = _public_status_for_role(routed_role, state)
        role_state["last_event_type"] = event_type
        role_state["last_source"] = source
    else:
        manager["fallback_worker"]["status"] = "active"
        manager["fallback_worker"]["last_event_type"] = event_type
        manager["fallback_worker"]["last_source"] = source
...
def agent_push():
    ...
    try:
        ...
        apply_manager_event({
            ...
            "role": _classify_agent_role(target.get("name", ""), detail, False),
            "state": state,
            "detail": detail,
        })
...
@app.route("/api/reply-drafts", methods=["GET"])
def api_reply_drafts():
    """Return recent reply drafts generated by runtime result.json files."""
    drafts = load_reply_drafts(limit=50)
...
def manager_event():
    """Route incoming events to dev/ops/research or fallback worker."""
    try:
...
if __name__ == "__main__":
    ...
    print("=" * 50)
    print("Deepnoa Office UI - Backend State Service")
    print("=" * 50)
